...
static inline int max77663_i2c_write(struct i2c_client *client, u8 addr, void *src, u32 bytes) {
	u8 buf[bytes + 1];
	int ret;
...
static inline int max77663_i2c_read(struct i2c_client *client, u8 addr, void *dest, u32 bytes) {
	int ret;
...
int max77663_read(struct device *dev, u8 addr, void *values, u32 len, bool is_rtc) {
...
int max77663_write(struct device *dev, u8 addr, void *values, u32 len, bool is_rtc) {
	struct max77663_chip *chip = dev_get_drvdata(dev);
	struct i2c_client *client = NULL;
	int ret;
...
int max77663_set_bits(struct device *dev, u8 addr, u8 mask, u8 value, bool is_rtc) {
	struct max77663_chip *chip = dev_get_drvdata(dev);
	struct i2c_client *client = NULL;
	u8 tmp;
	int ret;
...
static void max77663_power_off(void) {
	...
	max77663_set_bits(chip->dev, MAX77663_REG_ONOFF_CFG1, ONOFF_SFT_RST_MASK, ONOFF_SFT_RST_MASK, 0);
}
...
static inline int max77663_cache_write(struct device *dev, u8 addr, u8 mask, u8 val, u8 *cache) {
	u8 new_val;
	int ret;
...
static inline struct max77663_chip *max77663_chip_from_gpio(struct gpio_chip *gpio) {
	return container_of(gpio, struct max77663_chip, gpio);
}

static int max77663_gpio_set_pull_up(struct max77663_chip *chip, int offset, int pull_up) {
	u8 val = 0;
	...
	return max77663_cache_write(chip->dev, MAX77663_REG_GPIO_PU, (1 << offset), val, &chip->cache_gpio_pu);
}
...
static int max77663_gpio_set_pull_down(struct max77663_chip *chip, int offset, int pull_down) {
	...
	return max77663_cache_write(chip->dev, MAX77663_REG_GPIO_PD, (1 << offset), val, &chip->cache_gpio_pd);
}

static inline int max77663_gpio_is_alternate(struct max77663_chip *chip, int offset) {
...
int max77663_gpio_set_alternate(int gpio, int alternate) {
	...
	return max77663_cache_write(chip->dev, MAX77663_REG_GPIO_ALT, (1 << gpio), val, &chip->cache_gpio_alt);
}
...
static int max77663_gpio_dir_output(struct gpio_chip *gpio, unsigned offset, int value) {
	struct max77663_chip *chip = max77663_chip_from_gpio(gpio);
	...
	return max77663_cache_write(chip->dev, GPIO_REG_ADDR(offset), mask, val, &chip->cache_gpio_ctrl[offset]);
}

static int max77663_gpio_set_debounce(struct gpio_chip *gpio, unsigned offset, unsigned debounce) {
	struct max77663_chip *chip = max77663_chip_from_gpio(gpio);
...
static void max77663_gpio_set(struct gpio_chip *gpio, unsigned offset, int value) {
	struct max77663_chip *chip = max77663_chip_from_gpio(gpio);
	u8 val = (value ? 1 : 0) << GPIO_CTRL_DOUT_SHIFT;
	...
	max77663_cache_write(chip->dev, GPIO_REG_ADDR(offset), GPIO_CTRL_DOUT_MASK, val, &chip->cache_gpio_ctrl[offset]);
}
...
static void max77663_gpio_dbg_show(struct seq_file *s, struct gpio_chip *gpio) {
	...
	for (i = 0; i < gpio->ngpio; i++) {
		...
		seq_printf(s, " gpio-%-3d (%-20.20s) ", i + chip->gpio_base, label);
		...
		ret = max77663_read(chip->dev, GPIO_REG_ADDR(i), &ctrl_val, 1, 0);
		if (ret < 0) {
			seq_printf(s, "\n");
			continue;
		}
		...
		if (!is_out) {
			...
			dbnc = (ctrl_val & GPIO_CTRL_DBNC_MASK) >> GPIO_CTRL_DBNC_SHIFT;
			seq_printf(s, " debounce-%s",
				   dbnc ==  GPIO_DBNC_8MS ? "8ms" :
				   dbnc ==  GPIO_DBNC_16MS ? "16ms" :
				   dbnc ==  GPIO_DBNC_32MS ? "32ms" : "none");
		} else {
			seq_printf(s, " %s", (ctrl_val & GPIO_CTRL_OUT_DRV_MASK ? "output-drive" : "open-drain"));
		}
...
static int max77663_gpio_set_config(struct max77663_chip *chip, struct max77663_gpio_config *gpio_cfg) {
	...
	if (gpio_cfg->pull_up != GPIO_PU_DEF) {
		...
		if (ret < 0) {
			dev_err(chip->dev, "gpio_set_config: Failed to set gpio%d pull-up\n", gpio);
			return ret;
		}
	}

	if (gpio_cfg->pull_down != GPIO_PD_DEF) {
		ret = max77663_gpio_set_pull_down(chip, gpio, gpio_cfg->pull_down);
		if (ret < 0) {
			dev_err(chip->dev, "gpio_set_config: Failed to set gpio%d pull-down\n", gpio);
			return ret;
		}
	}
	...
	if (gpio_cfg->dir != GPIO_DIR_DEF) {
		...
		ret = max77663_cache_write(chip->dev, GPIO_REG_ADDR(gpio), mask, val, &chip->cache_gpio_ctrl[gpio]);
		if (ret < 0) {
			dev_err(chip->dev, "gpio_set_config: Failed to set gpio%d control\n", gpio);
			return ret;
		}
	}

	if (gpio_cfg->alternate != GPIO_ALT_DEF) {
		ret = max77663_gpio_set_alternate(gpio + chip->gpio_base, gpio_cfg->alternate);
		if (ret < 0) {
			dev_err(chip->dev, "gpio_set_config: Failed to set gpio%d alternate\n", gpio);
			return ret;
		}
	}
...
static int max77663_gpio_init(struct max77663_chip *chip) {
	...
	ret = max77663_read(chip->dev, MAX77663_REG_GPIO_CTRL0, chip->cache_gpio_ctrl, MAX77663_GPIO_NR, 0);
	if (ret < 0) {
		dev_err(chip->dev, "gpio_init: Failed to get gpio control\n");
		return ret;
	}

	ret = max77663_read(chip->dev, MAX77663_REG_GPIO_PU, &chip->cache_gpio_pu, 1, 0);
	if (ret < 0) {
		dev_err(chip->dev, "gpio_init: Failed to get gpio pull-up\n");
		return ret;
	}

	ret = max77663_read(chip->dev, MAX77663_REG_GPIO_PD, &chip->cache_gpio_pd, 1, 0);
	if (ret < 0) {
		dev_err(chip->dev, "gpio_init: Failed to get gpio pull-down\n");
		return ret;
	}

	ret = max77663_read(chip->dev, MAX77663_REG_GPIO_ALT, &chip->cache_gpio_alt, 1, 0);
	if (ret < 0) {
	...
	for (i = 0; i < chip->pdata->num_gpio_cfgs; i++) {
		ret = max77663_gpio_set_config(chip, &chip->pdata->gpio_cfgs[i]);
		if (ret < 0) {
			dev_err(chip->dev, "gpio_init: Failed to set gpio config\n");
			return ret;
		}
	}
...
static void max77663_irq_sync_unlock(struct irq_data *data) {
	struct max77663_chip *chip = irq_data_get_irq_chip_data(data);
	struct max77663_irq_data *irq_data = &max77663_irqs[data->irq - chip->irq_base];
	...
	if ((idx != -1) && (irq_mask != chip->cache_irq_mask[idx])) {
		...
		ret = max77663_write(chip->dev, irq_data->mask_reg, &irq_mask, len, irq_data->is_rtc);
		if (ret < 0) goto out;
		...
		ret = max77663_cache_write(chip->dev, GPIO_REG_ADDR(offset), GPIO_CTRL_REFE_IRQ_MASK, irq_mask, &chip->cache_gpio_ctrl[offset]);
		if (ret < 0) goto out;
	...
	if (update_irq_top && (irq_top_mask != chip->cache_irq_top_mask)) {
		ret = max77663_cache_write(chip->dev, MAX77663_REG_IRQ_TOP_MASK, irq_data->top_mask, irq_top_mask, &chip->cache_irq_top_mask);
		if (ret < 0) goto out;
	}
...
static inline int max77663_do_irq(struct max77663_chip *chip, u8 addr, int irq_base, int irq_end) {
...
static irqreturn_t max77663_irq(int irq, void *data) {
	...
	if (irq_top & IRQ_TOP_GLBL_MASK) {
		ret = max77663_do_irq(chip, MAX77663_REG_LBT_IRQ, IRQ_LBT_BASE, IRQ_LBT_END);
		if (ret < 0) return IRQ_NONE;
	}

	if (irq_top & IRQ_TOP_GPIO_MASK) {
		ret = max77663_do_irq(chip, MAX77663_REG_GPIO_IRQ, IRQ_GPIO_BASE, IRQ_GPIO_END);
		if (ret < 0) return IRQ_NONE;
	}

	if (irq_top & IRQ_TOP_ONOFF_MASK) {
		ret = max77663_do_irq(chip, MAX77663_REG_ONOFF_IRQ, IRQ_ONOFF_BASE, IRQ_ONOFF_END);
		if (ret < 0) return IRQ_NONE;
	}
...
static int max77663_irq_init(struct max77663_chip *chip) {
	...
	max77663_write(chip->dev, MAX77663_REG_IRQ_TOP_MASK, &chip->cache_irq_top_mask, 1, 0);
	max77663_write(chip->dev, MAX77663_REG_LBT_IRQ_MASK, &chip->cache_irq_mask[CACHE_IRQ_LBT], 1, 0);
	max77663_write(chip->dev, MAX77663_REG_SD_IRQ_MASK, &chip->cache_irq_mask[CACHE_IRQ_SD], 1, 0);
	max77663_write(chip->dev, MAX77663_REG_LDOX_IRQ_MASK, &chip->cache_irq_mask[CACHE_IRQ_LDO], 2, 0);
	max77663_write(chip->dev, MAX77663_REG_ONOFF_IRQ_MASK, &chip->cache_irq_mask[CACHE_IRQ_ONOFF], 1, 0);
	...
	for (i = chip->irq_base; i < (MAX77663_IRQ_NR + chip->irq_base); i++) {
		if (i >= NR_IRQS) {
			dev_err(chip->dev, "irq_init: Can't set irq chip for irq %d\n", i);
			continue;
		}

		irq_set_chip_data(i, chip);

		if ((IRQ_GPIO_BASE <= i - chip->irq_base) && (i - chip->irq_base <= IRQ_GPIO_END)) {
			irq_set_chip_and_handler(i, &max77663_irq_gpio_chip, handle_edge_irq);
		} else {
	...
	ret = request_threaded_irq(chip->i2c_power->irq, NULL, max77663_irq, IRQF_ONESHOT, "max77663", chip);
	if (ret) {
...
static int max77663_debugfs_dump_regs(struct max77663_chip *chip, char *label, u8 *addrs, int num_addrs, char *buf, ssize_t *len, int is_rtc) {
	ssize_t count = *len;
	u8 val;
	int ret = 0;
	int i;
...
static ssize_t max77663_debugfs_regs_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos) {
	...
	max77663_debugfs_dump_regs(chip, "[Global]", global_regs, ARRAY_SIZE(global_regs), buf, &len, 0);
	max77663_debugfs_dump_regs(chip, "[Step-Down]", sd_regs, ARRAY_SIZE(sd_regs), buf, &len, 0);
	max77663_debugfs_dump_regs(chip, "[LDO]", ldo_regs, ARRAY_SIZE(ldo_regs), buf, &len, 0);
	max77663_debugfs_dump_regs(chip, "[GPIO]", gpio_regs, ARRAY_SIZE(gpio_regs), buf, &len, 0);
	max77663_debugfs_dump_regs(chip, "[RTC]", rtc_regs, ARRAY_SIZE(rtc_regs), buf, &len, 1);
	max77663_debugfs_dump_regs(chip, "[32kHz Oscillator]", osc_32k_regs, ARRAY_SIZE(osc_32k_regs), buf, &len, 0);
	max77663_debugfs_dump_regs(chip, "[Backup Battery Charger]", bbc_regs, ARRAY_SIZE(bbc_regs), buf, &len, 0);
	max77663_debugfs_dump_regs(chip, "[On/OFF Controller]", onoff_regs, ARRAY_SIZE(onoff_regs), buf, &len, 0);
	max77663_debugfs_dump_regs(chip, "[Flexible Power Sequencer]", fps_regs, ARRAY_SIZE(fps_regs), buf, &len, 0);
	max77663_debugfs_dump_regs(chip, "[Chip Identification]", cid_regs, ARRAY_SIZE(cid_regs), buf, &len, 0);
...
static void max77663_debugfs_init(struct max77663_chip *chip) {
	max77663_dentry_regs = debugfs_create_file(chip->i2c_power->name, 0444, 0, chip, &max77663_debugfs_regs_fops);
	if (!max77663_dentry_regs) {
		dev_warn(chip->dev, "debugfs_init: Failed to create debugfs file\n");
	}
}
...
static int max77663_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	ret = mfd_add_devices(&client->dev, 0, pdata->sub_devices, pdata->num_subdevs, NULL, 0);
	if (ret != 0) {
